...
<form action="{{ route('polls.toggle-status', $poll) }}" method="POST">
    ...
    @if(!$poll->is_active)
        <button type="submit" title="Click to Publish" 
                class="px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-100 dark:border-amber-500/20 hover:bg-amber-100 transition">
            <i class="fas fa-eye-slash mr-1"></i> Hidden
        </button>
    @else
        <button type="submit" title="Click to Hide" 
                class="px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest bg-green-50 dark:bg-green-500/10 text-green-600 dark:text-green-400 border border-green-100 dark:border-green-500/20 hover:bg-green-100 transition">
